...
import { ebayPostBlandModelStatisticsRequestBody, Item } from './utils/types.ts'
...
export const addItemDetail = async (
  ...
  try {
    ...
    if (existTargetSoldData) {
      ...
      await supabaseClient.from('item_detail').insert([soldItemDetail])
    }
    if (existTargetUnSoldData) {
      ...
      await supabaseClient.from('item_detail').insert([unSoldItemDetail])
    }
